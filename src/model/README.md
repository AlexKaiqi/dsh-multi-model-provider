...
模型能读到的能力说明、工具描述和顶层参数说明集中在本目录；复用对象的嵌套字段说明
仍与结构 schema 一起保存在 `../tools.ts`。实现代码不得重复内联本目录已经定义的文案。
...
| `tool-surfaces.ts` | 十六个工具的模型可见表面 | 工具名、描述（正反向激活条件）、顶层参数描述、`helpPointer` |
...
`../tools.ts` 只装配工具名、工具描述和顶层参数描述，并为画像、证据、价格等
复用对象维护嵌套结构 schema；同一字段说明不得在两处重复定义。
