window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function targetFromTask(raw) {
			const row = object$1(raw);
			if (typeof row.id !== "string" || typeof row.provider !== "string" || typeof row.model !== "string") return void 0;
			const connection = object$1(row.connectionProfile);
			return {
				id: row.id,
				kind: "task",
				provider: row.provider,
		...
		function snapshotPortraitTargets(catalog) {
			const language = (Array.isArray(catalog?.languageModels) ? catalog.languageModels : []).map(targetFromLanguage).filter(Boolean);
			const task = (Array.isArray(catalog?.taskModels) ? catalog.taskModels : []).map(targetFromTask).filter(Boolean);
