window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const EN = {
			...
			portraitResearchCurrent: "Collect portrait",
			...
			portraitJobsUnavailable: "Portrait collection requires the dsh-temporary-session profile bundle. Install and enable it, then restart this profile.",
		...
		const ZH = {
			...
			portraitsEmpty: "先在当前 profile 注册或选择至少一个模型，画像目标才会出现在这里。",
			portraitResearchAll: "生成 / 刷新待完善画像",
			portraitResearchCurrent: "采集画像",
			...
			portraitJobsUnavailable: "画像采集需要 dsh-temporary-session profile bundle。请安装并启用后重启当前 profile。",
		...
		*/
		...
		function providerSelection(descriptor, connectionId) {
			const user = object$1(descriptor?.user);
			const models = object$1(object$1(user.providerProfiles)[connectionId] ?? object$1(user.connections)[connectionId]).models;
			if (!Array.isArray(models)) return void 0;
			return new Set(models.flatMap((row) => {
				const id = typeof row === "string" ? row : object$1(row).id;
				return typeof id === "string" && id.trim() ? [id] : [];
			}));
		}
		function selectedByProvider(selected, id, model) {
			const voice = typeof object$1(model.profile).voice === "string" ? object$1(model.profile).voice : void 0;
			return selected.has(id) || selected.has(model.model) || voice !== void 0 && selected.has(voice);
		}
		function userRegisteredTaskModel(value) {
			const model = object$1(value);
		...
		function userSelectedTaskModel(value) {
			return object$1(value).enabled === true;
		}
		...
		function snapshotPortraitTargets(multi, llm) {
			...
			const userModels = object$1(object$1(multi?.user).models);
			const connections = object$1(root.connections);
			const task = Object.entries(object$1(root.models)).flatMap(([id, raw]) => {
				const model = object$1(raw);
				const connection = object$1(connections[model.connection]);
				const selected = providerSelection(multi, model.connection);
				const userModel = userModels[id];
				if (!(userRegisteredTaskModel(userModel) || userSelectedTaskModel(userModel) || selected !== void 0 && selectedByProvider(selected, id, model))) return [];
				return [{
					id,
					kind: "task",
					...
					task: model.task,
					enabled: selected === void 0 ? model.enabled !== false : selectedByProvider(selected, id, model),
				...
				children: [
					...
					portraitTab === "collect" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							...
							disabled: !portraitJob.available || jobBusy || !target,
							onClick: () => target && void portraitJob.start("research", [target.id]),
							children: t("portraitStartCollection")
						}) }),
						portraitJob.job && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "mmp-job",
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { children: t(`portraitJob.${portraitJob.job.status}`) }),
								jobDetail && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: "mmp-muted",
									children: jobDetail
								}),
								portraitJob.job.sessionId && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: "mmp-button",
									onClick: () => sessions.open(portraitJob.job.sessionId),
									children: t("portraitOpenSession")
								}) })
							]
						}),
