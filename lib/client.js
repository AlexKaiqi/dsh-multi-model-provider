...
`;
		...
		/** Read the redacted settings document through DSH's typed Settings Remote. */
		function describeSettings(api) {
			return api.settings.describe().then(responseValue);
		}
		function useConfig(api, includeModelCatalog = false) {
			...
			const load = async () => {
				...
				try {
					const [settings, modelCatalog] = await Promise.all([describeSettings(api), includeModelCatalog ? fetch(MODEL_CATALOG_PATH, {
