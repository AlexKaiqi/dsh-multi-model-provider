...
function sendJson$2(res, status, value) {
	...
	ctx.inject(["modelCatalog", "webServer"], (scope) => {
		...
		catalogScope.effect(() => catalogScope.webServer.register({
			...
			handler: async (req, res) => {
				...
				try {
					sendJson$2(res, 200, await catalogScope.modelCatalog.snapshot());
				} catch (error) {
					sendJson$2(res, 500, { error: error instanceof Error ? error.message : String(error) });
				}
			}
		}));
	});
		...
		probeScope.effect(() => probeScope.webServer.register({
			...
			handler: async (req, res) => {
				...
				try {
					...
					sendJson$1(res, 200, await runPaidModelProbe(probeScope.llm, provider, model));
				} catch (error) {
					sendJson$1(res, 502, {
						ok: false,
...
function buildPortraitResearchPrompt(manifest) {
...
function buildPortraitProbePrompt(ids, approvedAt) {
...
const FETCH_PORTRAIT_SOURCE_TOOL = "fetch_portrait_source";
...
async function fetchPortraitSource(url, allowedSources, signal) {
	...
	if (response.status >= 300 && response.status < 400) throw new Error("portrait source redirects are not followed; add the final reviewed URL to the research plan");
...
function portraitSourceTool(allowedSources) {
	...
	return defineTool({
		...
		parameters: { url: {
			type: "string",
			required: true,
			description: "Exact URL copied from researchPlan.suggestedSources."
		} },
		output: {
			schema: { type: "json" },
			render: (_args, value) => [{
...
//#endregion
//#region lib/types/portrait-jobs.js
const PORTRAIT_JOBS_PATH = "/dsh-multi-model-provider/portrait-jobs";
const PORTRAIT_JOB_HEADER = "x-dsh-portrait-job";
var PortraitJobError = class extends Error {
	status;
...
async function readJson(req) {
	...
	for await (const chunk of req) {
		const bytes = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
		size += bytes.length;
		if (size > 16384) throw new PortraitJobError(413, "request body is too large");
		chunks.push(bytes);
	}
	const parsed = JSON.parse(Buffer.concat(chunks).toString("utf8") || "{}");
...
function outcome(events, firstSeq) {
	...
	for (const event of events) {
		...
		if (event.type === "assistant/message") {
			...
			if (text) summary = text.slice(0, 4e3);
		}
		if (event.type === "turn/end") reason = event.data.reason;
	}
	const completed = reason?.kind === "completed";
	const error = completed ? void 0 : typeof reason?.error?.message === "string" ? String(reason.error.message) : `background Agent stopped with '${String(reason?.kind ?? "unknown")}'`;
	return {
		summary,
		completed,
		...error === void 0 ? {} : { error }
...
function boundPortraitResearchTools(tools, targetIds) {
	...
	return Object.entries(RESEARCH_TOOL_NAMES).map(([sourceName, name]) => {
		...
		if (tool === void 0) throw new Error(`portrait research wrapper source '${sourceName}' is unavailable`);
		return {
			...tool,
			name,
			...
			execute: async (args, exec) => {
				...
				const bounded = sourceName === "validate_model_portrait" ? {
					...args,
					liveProbe: false
				} : args;
				return tool.execute(bounded, exec);
			}
		};
	});
}
function portraitResearchTools(ctx, targetIds) {
	return boundPortraitResearchTools(modelManagerTools(ctx), targetIds);
}
async function waitUntilIdle(agent, signal, timeoutMs = 18e5) {
	let timer;
	let abort;
	const interrupted = new Promise((_resolve, reject) => {
		abort = () => reject(/* @__PURE__ */ new Error("portrait background job was cancelled"));
		if (signal.aborted) abort();
		else signal.addEventListener("abort", abort, { once: true });
	});
	const timedOut = new Promise((_resolve, reject) => {
		timer = setTimeout(() => reject(/* @__PURE__ */ new Error("portrait background job timed out")), timeoutMs);
	});
	try {
		await Promise.race([
			agent.whenIdle(),
...
var PortraitJobCoordinator = class {
	ctx;
	latest;
	activeHandle;
	activeRun;
	...
	async start(action, ids, approved) {
		if (this.disposed) throw new PortraitJobError(503, "portrait job runner is unavailable");
		if (this.starting || this.latest !== void 0 && (this.latest.status === "queued" || this.latest.status === "running")) throw new PortraitJobError(409, "a portrait background job is already running");
		if (action === "probe" && (!approved || ids === void 0)) throw new PortraitJobError(400, "live probes require explicit approval and exact target ids");
		this.starting = true;
		let prompt;
		let selectedIds;
		let researchSources = [];
		try {
			if (action === "research") {
				const prepared = await prepareModelPortraits(this.ctx, {
					...ids === void 0 ? {} : { ids },
					includeDisabled: false
				});
				const candidates = Array.isArray(prepared.candidates) ? prepared.candidates : [];
				selectedIds = candidates.flatMap((candidate) => typeof candidate === "object" && candidate !== null && typeof candidate.id === "string" ? [candidate.id] : []);
				const manifest = {
					candidates,
				...
				selectedIds = ids;
				prompt = buildPortraitProbePrompt(selectedIds, (/* @__PURE__ */ new Date()).toISOString());
			}
		} finally {
			this.starting = false;
		}
		...
		this.latest = {
			...
			...selectedIds.length === 0 ? {
				status: "completed",
				finishedAt: now,
				summary: "No configured portraits currently need research."
			} : {}
		};
		if (selectedIds.length > 0) {
			...
			this.activeRun = run;
			run.finally(() => {
				if (this.activeRun === run) this.activeRun = void 0;
	...
	async dispose() {
		...
		await this.activeHandle?.dispose().catch(() => void 0);
		await this.activeRun?.catch(() => void 0);
		this.activeHandle = void 0;
		this.activeRun = void 0;
		this.activeAbort = void 0;
	}
	async execute(prompt, action, researchSources, signal) {
		...
		try {
			...
			const handle = await this.ctx.agents.create({
				...
				setup: (agentCtx) => {
					...
					const requiredTools = action === "research" ? [FETCH_PORTRAIT_SOURCE_TOOL, ...Object.values(RESEARCH_TOOL_NAMES)] : ["get_model_portrait", "validate_model_portrait"];
			...
			if (signal.aborted) throw new Error("portrait background job was cancelled");
			if (!(await this.ctx.temporaryWorkspaces.adopt({
				reservationId: workspace.reservationId,
				sessionId: String(sessionId)
			})).found) throw new Error("temporary Workspace reservation expired before Session adoption");
			...
			if (reservationId !== void 0 && !reservationAdopted) {
				const operation = sessionCreated ? "retain" : "discard";
				await this.ctx.temporaryWorkspaces[operation]({ reservationId }).catch(() => void 0);
			}
			...
			if (failure === void 0) {
				...
				if (completionSummary !== void 0) job.summary = completionSummary;
			} else {
				job.status = "failed";
...
function registerPortraitJobRoutes(ctx) {
	ctx.inject([
		"agents",
		"agentDefaultModel",
		"webServer",
		...
		jobScope.effect(() => {
			const unregister = jobScope.webServer.register({
				...
				handler: async (req, res) => {
					...
					if (req.headers["x-dsh-portrait-job"] !== "1" || !isSameOriginHttpRequest(req)) {
						sendJson(res, 403, {
							ok: false,
							error: "portrait jobs require a same-origin Settings request"
						});
						return;
					}
					try {
						const body = await readJson(req);
						if (body.action !== "research") throw new PortraitJobError(400, "Settings portrait jobs only run research; live probes require separate explicit Agent approval");
						sendJson(res, 202, {
							ok: true,
							job: await coordinator.start("research", targetIds(body.ids), false)
						});
					} catch (error) {
						sendJson(res, error instanceof PortraitJobError ? error.status : 500, {
							ok: false,
							error: error instanceof Error ? error.message : String(error)
						});
					}
