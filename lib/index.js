...
const FETCH_PORTRAIT_SOURCE_SURFACE = {
...
function modelProfile(input, index) {
	...
	return {
		...
		...requestMaxTokens === void 0 ? {} : { maxTokens: requestMaxTokens },
...
async function configureModelRoute(ctx, input) {
	...
	const providerExists = nestedValue(descriptor.value, ["providers", provider]) !== void 0;
	const ops = fields.length === 0 ? providerExists ? [] : [{
		op: "set",
	...
	const capabilityOnlyModels = input.models?.filter((model) => model.maxTokens !== void 0 && model.requestMaxTokens === void 0).map((model) => model.id.trim()) ?? [];
	return {
		provider,
		saved: true,
		...
		...capabilityOnlyModels.length === 0 ? {} : { warnings: [{
...
function portraitChecks(portrait) {
	...
	checks.push({
		id: "portrait.pricing",
		status: rates.length === 0 ? "warn" : "pass",
		message: rates.length === 0 ? "pricing is unknown" : "pricing rates are present"
	});
	checks.push({
		id: "portrait.performance.speed",
		status: performance.speedClass === void 0 ? "warn" : "pass",
		message: performance.speedClass === void 0 ? "speed class is unknown" : "speed class is present"
	});
	...
	if (performance.typicalLatencyMs !== void 0) checks.push({
...
function normalizeStoredPortrait(input) {
	const normalized = normalizePortrait(input);
	const stored = input.validation;
	if (stored === void 0) return normalized;
	return {
		...normalized,
		validation: {
			state: stored.state ?? normalized.validation.state,
			...stored.checkedAt === void 0 ? {} : { checkedAt: stored.checkedAt },
...
const PORTRAITS$1 = new Map(SPECS$1.map((spec) => [`${spec.provider}/${spec.model}`, researchedPortrait(spec)]));
...
const CURATED_LLM_PORTRAIT_IDS = [...PORTRAITS$1.keys()].map((id) => `llm:${id}`);
...
function builtinLlmPortrait(provider, model) {
	...
	const portrait = PORTRAITS$1.get(`${provider.trim()}/${normalizedModel}`) ?? PORTABLE_PORTRAITS.get(normalizedModel);
...
function llmObservations(events) {
	...
	for (const event of events) {
		const data = object$1(event.data);
		if (data === void 0) continue;
		if (event.type === "request/header") {
			const config = object$1(object$1(data.header)?.config);
		...
		const usage = object$1(data.usage);
...
const H3_SPEC = SPECS.find((spec) => spec.provider === "minimax" && spec.model === "MiniMax-H3" && spec.task === "video-generation");
const PORTABLE_H3 = portableH3Portrait();
const CURATED_TASK_MODEL_PORTRAIT_IDS = SPECS.map((spec) => `${spec.task}:${spec.provider}/${spec.model}`);
...
function builtinTaskPortrait(provider, model, task) {
	const normalizedModel = model.trim();
	const portrait = PORTRAITS.get(identity(provider.trim(), normalizedModel, task)) ?? (task === "video-generation" && ["MiniMaxAI/MiniMax-H3", "MiniMax-H3"].includes(normalizedModel) ? PORTABLE_H3 : void 0);
	return portrait === void 0 ? void 0 : structuredClone(portrait);
}
...
const DOUBAO_REALTIME_BASE_URL = "wss://openspeech.bytedance.com/api/v3/duplex/realtime/dialogue";
/**
...
*/
const DOUBAO_SPEECH_AGENT_PLAN_CATALOG = [{
	...
	registration: {
		enabled: false,
		connection: "doubao-speech-agent-plan",
		model: "volc.seedasr.sauc.duration",
		displayName: "豆包流式语音识别 2.0（Agent Plan）",
		task: "transcription",
		runtimeAdapter: "doubao-speech-agent-plan",
		credentialNames: ["apiKey"],
		input: ["audio", "file"],
		output: ["text"],
		execution: "streaming",
		capabilities: ["speech.transcribe.stream", "speech.transcribe.file"],
		operations: ["transcribe-stream", "transcribe-file"],
		roles: ["speech-to-text"],
		profile: {
			billingMode: "agent-plan",
			resourceId: "volc.seedasr.sauc.duration",
			requiresExplicitSelection: true,
			endpoint: "wss://openspeech.bytedance.com/api/v3/plan/sauc/bigmodel_async",
			finalResultEndpoint: "wss://openspeech.bytedance.com/api/v3/plan/sauc/bigmodel_nostream",
			protocol: "websocket-binary-v1-gzip"
		}
	}
}, {
	...
	registration: {
		...
		connection: "doubao-speech-agent-plan",
		model: "seed-tts-2.0",
		displayName: "豆包语音合成 2.0（Agent Plan HTTP）",
		task: "speech-synthesis",
		runtimeAdapter: "doubao-speech-agent-plan",
		credentialNames: ["apiKey"],
		input: ["text"],
...
const TASK_MODEL_SETTINGS_NAMESPACE = "multi-model-provider";
...
function assertSecretFreeMetadata(value, name, depth = 0) {
	if (depth > 8) throw new ModelManagerError(`${name} is nested too deeply`, "INVALID_TASK_MODEL_CONFIGURATION");
	if (typeof value === "string") {
		if (SECRET_METADATA_VALUE.test(value.trim())) throw new ModelManagerError(`${name} appears to contain a credential value; store only credential references`, "SECRET_VALUE_NOT_ALLOWED");
		return;
	}
	if (value === null || typeof value !== "object") return;
	if (Array.isArray(value)) {
		if (value.length > 1e3) throw new ModelManagerError(`${name} is too large`, "INVALID_TASK_MODEL_CONFIGURATION");
	...
	for (const [key, item] of entries) {
		if (SECRET_METADATA_KEY.test(key)) throw new ModelManagerError(`${name}.${key} is secret-shaped; profiles must contain non-secret metadata only`, "SECRET_VALUE_NOT_ALLOWED");
...
const TASK_MODEL_REGISTRY_SCHEMA = z.object({
	...
	providerProfiles: z.dict(connectionSchema).default({}).description("Provider profiles added through the Models settings page."),
...
function resolvedConfig(descriptor) {
	...
	return {
		...value,
		connections: Object.fromEntries(Object.entries(value.connections).map(([id, connection]) => [id, profiles[id] === void 0 ? connection : {
			...connection,
			...profiles[id]
		}]))
	};
...
*/
function providerModelSelection(descriptor, connectionId) {
	if (typeof descriptor.user !== "object" || descriptor.user === null || Array.isArray(descriptor.user)) return void 0;
	const user = descriptor.user;
	const connections = user.providerProfiles ?? user.connections;
	if (typeof connections !== "object" || connections === null || Array.isArray(connections)) return void 0;
	const connection = connections[connectionId];
	if (typeof connection !== "object" || connection === null || Array.isArray(connection)) return void 0;
...
function selectedByProviderEditor(selected, routeId, model) {
	const voice = typeof model.profile?.voice === "string" ? model.profile.voice : void 0;
	return selected.has(routeId) || selected.has(model.model) || voice !== void 0 && selected.has(voice);
}
function userRegisteredTaskModel(value) {
	if (typeof value !== "object" || value === null || Array.isArray(value)) return false;
	const model = value;
	return typeof model.connection === "string" && typeof model.model === "string" && typeof model.task === "string";
}
function userSelectedTaskModel(value) {
	return typeof value === "object" && value !== null && !Array.isArray(value) && value.enabled === true;
}
/**
...
*/
function configuredTaskModelIds(ctx) {
	const descriptor = requiredDescriptor(ctx);
	const config = resolvedConfig(descriptor);
	const user = typeof descriptor.user === "object" && descriptor.user !== null && !Array.isArray(descriptor.user) ? descriptor.user : {};
	const userModels = typeof user.models === "object" && user.models !== null && !Array.isArray(user.models) ? user.models : {};
	...
	for (const [id, registration] of Object.entries(config.models)) {
		...
		if (userRegisteredTaskModel(userModels[id]) || userSelectedTaskModel(userModels[id]) || selected !== void 0 && selectedByProviderEditor(selected, id, registration)) configured.add(id);
	}
	return configured;
}
function resolveTaskModelRoute(ctx, id, descriptor = requiredDescriptor(ctx)) {
...
async function effectiveTaskModelAvailability(ctx, routeOrId, descriptor = requiredDescriptor(ctx)) {
	...
	const credential = selectedRefs.credentialRef === void 0 ? void 0 : await credentialStatus(ctx, selectedRefs.credentialRef);
	const credentials = await namedCredentialStatuses(ctx, selectedRefs.credentialRefs);
	const credentialReady = credential?.configured !== false && Object.values(credentials ?? {}).every((item) => item.configured);
	const providerSelection = providerModelSelection(descriptor, route.registration.connection);
	const enabled = providerSelection === void 0 ? route.registration.enabled !== false : selectedByProviderEditor(providerSelection, route.id, route.registration);
	const adapterAvailable = route.registration.execution === "realtime" ? ctx.realtimeModelRuntime?.hasAdapter(route.registration.runtimeAdapter) ?? false : ctx.taskModelRuntime?.hasAdapter(route.registration.runtimeAdapter, route) ?? false;
	return {
		route,
		enabled,
		adapterAvailable,
...
function validateConnection(id, connection) {
	...
	if (catalogCredentialName === "default" && optionalText$1(connection.credentialRef) === void 0) throw new ModelManagerError(`connections.${id}.catalogCredentialName references missing default credentialRef`, "INVALID_TASK_MODEL_CONFIGURATION");
...
function validateTaskModelRegistry(config) {
	...
	for (const [id, model] of Object.entries(config.models)) {
		...
		for (const name of credentialNames ?? []) if (name === "default" ? connection.credentialRef === void 0 : connection.credentialRefs?.[name] === void 0) throw new ModelManagerError(`models.${id}.credentialNames references unknown connection credential slot '${name}'`, "INVALID_TASK_MODEL_CONFIGURATION");
...
function catalogCredentialRef(connection) {
	const name = optionalText$1(connection.catalogCredentialName);
	if (name === void 0) return void 0;
	return name === "default" ? connection.credentialRef : connection.credentialRefs?.[name];
}
function catalogOrigin(connection) {
	try {
...
async function assertCatalogCredentialBinding(ctx, existing, next) {
	const nextRef = catalogCredentialRef(next);
	if (nextRef === void 0) return;
	if (!(await credentialStatus(ctx, nextRef)).configured) return;
	if (!(existing !== void 0 && catalogCredentialRef(existing) === nextRef && catalogOrigin(existing) === catalogOrigin(next))) throw new ModelManagerError(`configured credential '${nextRef}' cannot be rebound to a new task-catalog origin by an Agent tool; create the endpoint with an unconfigured reference, then configure it in secure Settings`, "TASK_MODEL_CATALOG_CREDENTIAL_REBIND_FORBIDDEN");
}
async function registerTaskModel(ctx, input) {
	...
	await assertCatalogCredentialBinding(ctx, existingConnection, {
		...existingConnection ?? {},
		...connectionFields
	});
	const modelFields = {
...
function privateIp(address) {
	...
	return mapped !== void 0 && privateIpv4(mapped);
}
async function assertSafeCatalogEndpoint(endpoint) {
	const url = new URL(endpoint);
	if (url.username !== "" || url.password !== "") throw new ModelManagerError("task catalog URLs must not contain inline credentials", "TASK_MODEL_CATALOG_ENDPOINT_FORBIDDEN");
	const hostname = url.hostname.toLowerCase().replace(/\.$/, "");
	if (hostname === "localhost" || hostname.endsWith(".localhost") || hostname.endsWith(".local") || hostname === "metadata.google.internal" || hostname === "metadata") throw new ModelManagerError("task catalog endpoints must not target local or metadata hosts", "TASK_MODEL_CATALOG_ENDPOINT_FORBIDDEN");
	if (isIP(hostname) !== 0 && privateIp(hostname)) throw new ModelManagerError("task catalog endpoints must not target private, loopback, link-local, or multicast addresses", "TASK_MODEL_CATALOG_ENDPOINT_FORBIDDEN");
	if (!hostname.endsWith(".test") && !hostname.endsWith(".example") && isIP(hostname) === 0) {
		...
		if (addresses.length === 0 || addresses.some((item) => privateIp(item.address))) throw new ModelManagerError("task catalog hostname resolves to a private, loopback, link-local, or multicast address", "TASK_MODEL_CATALOG_ENDPOINT_FORBIDDEN");
	}
}
...
async function discoverTaskModels(ctx, input, signal = AbortSignal.timeout(15e3)) {
	...
	if (response.status >= 300 && response.status < 400) throw new ModelManagerError("model catalog redirects are not followed; configure the final trusted endpoint explicitly", "TASK_MODEL_CATALOG_REDIRECT_FORBIDDEN");
	if (!response.ok) throw new ModelManagerError(`model catalog request failed with HTTP ${response.status}`, "TASK_MODEL_CATALOG_REQUEST_FAILED");
	const body = JSON.parse(await boundedResponseText(response, 2097152));
...
async function selectTaskModels(ctx, input) {
	...
	if (providerModelSelection(descriptor, connectionId) !== void 0) {
		const providerModels = routes.flatMap(([id, model]) => {
			if (!selected.has(id)) return [];
			return [{
				id: (typeof model.profile?.voice === "string" ? model.profile.voice : void 0) ?? model.model,
				...model.displayName === void 0 ? {} : { name: model.displayName }
			}];
		});
		const profileRoot = typeof descriptor.user === "object" && descriptor.user !== null && !Array.isArray(descriptor.user) && typeof descriptor.user.providerProfiles === "object" ? "providerProfiles" : "connections";
		ops.push({
			op: "set",
			path: [
				profileRoot,
...
async function listTaskModels(ctx, input = {}) {
	...
	for (const [id, model] of Object.entries(config.models)) {
		...
		const { enabled, adapterAvailable, callable, credential: status, credentials: statuses } = await effectiveTaskModelAvailability(ctx, {
			id,
			connection,
			registration: model
		}, descriptor);
		const bundledPortrait = builtinTaskPortrait(connection.provider, model.model, model.task);
		const portrait = model.portrait === void 0 ? bundledPortrait : normalizeStoredPortrait(model.portrait);
		models.push({
			id,
			enabled,
			provider: connection.provider,
...
async function mutatePortraitSettings(ctx, operations, expectedRevision) {
...
async function resolvePortraitTarget(ctx, id, signal) {
	...
	if (config.models[targetId] !== void 0) {
		...
		return {
			kind: "task",
			id: route.id,
			portrait: route.registration.portrait === void 0 ? bundled : normalizeStoredPortrait(route.registration.portrait),
	...
	return {
		...
		model: parsed.model,
		portrait: binding?.portrait === void 0 ? bundled : normalizeStoredPortrait(binding.portrait),
...
async function upsertModelPortrait(ctx, input, expectedRevision) {
...
async function snapshotModelCatalog(ctx) {
	...
	for (const providerRow of asRecordList(routes.providers)) {
		...
		if (modelError) languageFailures.push({
			id: provider,
			name: providerName,
			message: modelError
		});
...
//#endregion
//#region lib/types/catalog-route.js
...
function registerModelCatalogRoute(ctx) {
	ctx.inject(["modelCatalog", "webServer"], (scope) => {
		...
		catalogScope.effect(() => catalogScope.webServer.register({
			...
			handler: async (req, res) => {
				...
				try {
					sendJson$1(res, 200, await catalogScope.modelCatalog.snapshot());
				} catch (error) {
					sendJson$1(res, 500, { error: error instanceof Error ? error.message : String(error) });
				}
			}
		}));
	});
}
const DOUBAO_REALTIME_MODEL = "1.2.6.1";
/**
...
*/
function probeDoubaoRealtimeKey(options) {
	const apiKey = String(options.apiKey ?? "").trim();
	if (apiKey === "") throw new Error("请输入豆包 Realtime API Key 后再加载音色。");
	const endpoint = new URL(String(options.endpoint ?? "wss://openspeech.bytedance.com/api/v3/duplex/realtime/dialogue"));
	const trustedOrigins = options.trustedOrigins ?? /* @__PURE__ */ new Set(["wss://openspeech.bytedance.com"]);
	if (!["wss:", "ws:"].includes(endpoint.protocol) || !trustedOrigins.has(endpoint.origin)) throw new Error(`豆包 Realtime 端点不受信任：${endpoint.origin}`);
	if (options.signal?.aborted) throw options.signal.reason ?? /* @__PURE__ */ new Error("豆包 Realtime 鉴权已取消。");
	return new Promise((resolve, reject) => {
		const socket = new WebSocket(endpoint, {
			headers: { "X-Api-Key": apiKey },
			handshakeTimeout: options.timeoutMs ?? 15e3,
			maxPayload: 524288
		});
		let settled = false;
		const finish = (error) => {
		...
		const abort = () => finish(options.signal?.reason instanceof Error ? options.signal.reason : /* @__PURE__ */ new Error("豆包 Realtime 鉴权已取消。"));
		const timer = setTimeout(() => finish(/* @__PURE__ */ new Error("豆包 Realtime 鉴权超时。")), options.timeoutMs ?? 15e3);
			...
			session: {
				...
				audio: {
					...
					output: {
						...
						voice: DOUBAO_REALTIME_VOICES[0].voice
					}
				},
				tools: []
			}
		...
		socket.on("message", (data, binary) => {
			if (binary) return finish(/* @__PURE__ */ new Error("豆包 Realtime 鉴权收到意外的二进制响应。"));
			try {
		...
		socket.on("unexpected-response", (_request, response) => finish(/* @__PURE__ */ new Error(`豆包 Realtime 鉴权失败：HTTP ${String(response.statusCode ?? "unknown")}`)));
		socket.on("error", (error) => finish(/* @__PURE__ */ new Error(`豆包 Realtime 连接失败：${error.message}`)));
		socket.on("close", (code, reason) => {
			if (!settled) finish(/* @__PURE__ */ new Error(`豆包 Realtime 在鉴权完成前关闭：${reason.toString() || `code ${code}`}`));
		});
	});
}
/** Validate the draft key live, then return the documented voice directory. */
async function discoverDoubaoRealtimeVoices(request) {
	await probeDoubaoRealtimeKey({
		...request.baseURL === void 0 ? {} : { endpoint: request.baseURL },
...
var TaskModelRuntime = class extends Service {
	...
	async probe(route, signal) {
		...
		if (adapter.probe === void 0) return {
			supported: false,
			ok: false,
			message: `adapter '${adapter.id}' exposes no live probe; provider reachability was not measured`
		};
...
//#endregion
//#region lib/types/pipeline-runtime.js
/** Internal plugin-to-plugin execution facade; unlike invoke_task_model it carries no tool/session authority. */
var TaskPipelineRuntime = class extends Service {
	constructor(ctx) {
		super(ctx, "taskPipelineRuntime");
	}
	async invoke(expectedTask, input, signal) {
		const route = resolveTaskModelRoute(this.ctx, input.routeId);
		if (route.registration.task !== expectedTask) throw new ModelManagerError(`task route '${route.id}' serves '${route.registration.task}', expected '${expectedTask}'`, "TASK_PIPELINE_STAGE_MISMATCH");
		if (route.registration.execution === "realtime") throw new ModelManagerError(`task route '${route.id}' is realtime and cannot be used as a composed stage`, "TASK_PIPELINE_REALTIME_STAGE");
		...
		if (!route.registration.operations.includes(operation)) throw new ModelManagerError(`task route '${route.id}' does not declare '${operation}'`, "UNSUPPORTED_TASK_MODEL_OPERATION");
...
var RealtimeModelRuntime = class extends Service {
	...
	async models() {
		const descriptor = this.ctx.settings.describe({ redactSecrets: true }).find((item) => item.ns === TASK_MODEL_SETTINGS_NAMESPACE);
		if (descriptor === void 0) throw new ModelManagerError("multi-model-provider settings are unavailable; register the plugin before listing realtime models", "TASK_MODEL_SETTINGS_UNAVAILABLE");
	...
	async model(routeId, protocol = "") {
		...
		const route = candidates.find((candidate) => candidate.id === selected) ?? candidates.find((candidate) => candidate.model === selected);
		if (route === void 0) throw new ModelManagerError(`unknown realtime model '${selected}'`, "UNKNOWN_REALTIME_MODEL");
		return route;
	}
...
async function invokeTaskModel(ctx, input, exec) {
	...
	if (route.registration.execution === "realtime") throw new ModelManagerError(`task model '${route.id}' is a realtime route; use realtimeModelRuntime instead of invoke_task_model`, "REALTIME_TASK_MODEL_REQUIRES_RUNTIME");
	const availability = await effectiveTaskModelAvailability(ctx, route);
	if (!availability.enabled) throw new ModelManagerError(`task model '${route.id}' is registered but disabled by the current model selection`, "TASK_MODEL_DISABLED");
	if (!availability.adapterAvailable) throw new ModelManagerError(`task model '${route.id}' runtime adapter '${route.registration.runtimeAdapter ?? "undeclared"}' is unavailable`, "TASK_MODEL_ADAPTER_UNAVAILABLE");
	if (!availability.credentialReady) throw new ModelManagerError(`task model '${route.id}' requires configured credential references`, "TASK_MODEL_CREDENTIAL_MISSING");
	const operation = input.operation.trim();
...
//#endregion
//#region lib/types/volcengine-defaults.js
/** Shared by the Host tools and the native Models-page setup form. */
const VOLCENGINE_PROVIDER = "volcengine";
const VOLCENGINE_AGENT_PLAN_PROVIDER = "volcengine-agent-plan";
const VOLCENGINE_ARK_PLAN_BASE_URL = "https://ark.cn-beijing.volces.com/api/plan/v3";
const VOLCENGINE_ARK_PAYG_BASE_URL = "https://ark.cn-beijing.volces.com/api/v3";
const VOLCENGINE_ARK_BASE_URL = VOLCENGINE_ARK_PAYG_BASE_URL;
const VOLCENGINE_ARK_API = "openai-completions";
const VOLCENGINE_ARK_API_KEY = "ARK_API_KEY";
//#endregion
...
const LEGACY_VOLCENGINE_ARK_API_KEY = "VOLCENGINE_API_KEY";
/** Copy the pre-ARK credential reference forward without exposing or deleting its value. */
async function migrateLegacyVolcengineCredential(ctx) {
	...
	if (!(await ctx.credentials.describe(legacy)).configured) return false;
	if (!current.writable) throw new ModelManagerError(`${CREDENTIALS.arkApiKey} is read-only; cannot migrate ${LEGACY_VOLCENGINE_ARK_API_KEY}`, "VOLCENGINE_CREDENTIAL_MIGRATION_UNAVAILABLE");
...
function selectedLanguageModels(ctx, provider) {
	const profile = object(object(object(descriptor(ctx, PI_AI_SETTINGS_NAMESPACE)?.value)?.providers)?.[provider]);
...
async function discoverVolcengineModels(ctx, request) {
	const profile = object(object(object(descriptor(ctx, PI_AI_SETTINGS_NAMESPACE)?.value)?.providers)?.[VOLCENGINE_PROVIDER]);
	const baseURL = request.baseURL?.trim() || (typeof profile?.baseURL === "string" ? profile.baseURL : "https://ark.cn-beijing.volces.com/api/v3");
	let apiKey = request.apiKey?.trim();
	if (!apiKey) {
		const storedURL = typeof profile?.baseURL === "string" ? profile.baseURL : VOLCENGINE_ARK_BASE_URL;
		if (baseURL.replace(/\/+$/, "") !== storedURL.replace(/\/+$/, "")) throw new ModelManagerError("Enter an API key to query a different endpoint; the saved key is not forwarded to it.", "ARK_CREDENTIAL_REQUIRED");
...
*/
async function runPaidModelProbe(llm, provider, model, signal) {
	await llm.resolveModelInfo(provider, model);
	const started = performance.now();
	let firstTokenAt;
	let finish;
	const probeSignal = signal === void 0 ? AbortSignal.timeout(2e4) : AbortSignal.any([signal, AbortSignal.timeout(2e4)]);
...
function probeSpeedClass(latencyMs) {
	return latencyMs <= 1e3 ? "instant" : latencyMs <= 2500 ? "fast" : latencyMs <= 6e3 ? "balanced" : "slow";
}
function portraitWithProbe(portrait, probe) {
	...
	return {
		...
		performance: {
			...
			lastProbe: {
				...
				...probe.timeToFirstTokenMs === void 0 ? {} : { timeToFirstTokenMs: probe.timeToFirstTokenMs }
			}
		},
		evidence: [...portrait.evidence.filter((item) => item.id !== "runtime-probe:latest"), {
			...
			...probe.message === void 0 ? {} : { notes: probe.message }
		}]
	};
}
...
async function validateModelPortrait(ctx, input, signal) {
	...
	if (target.kind === "llm") {
		...
		if (input.liveProbe === true) {
			...
			try {
				...
				liveCheck = {
					...
					status: "pass",
					message: `Agent live probe passed in ${probe.latencyMs} ms`
				};
			} catch (error) {
		...
		const checks = [...portraitChecks(workingPortrait)];
		...
		if (liveCheck !== void 0) checks.push(liveCheck);
		const validated = {
			...workingPortrait,
			validation: {
	...
	let liveCheck;
	if (input.liveProbe === true) {
		if (route.registration.execution === "realtime") liveCheck = {
			id: "runtime.live-probe",
			status: "warn",
			message: "realtime routes are probed through realtimeModelRuntime, not the generic task-model probe"
		};
		else {
			const started = performance.now();
			try {
				const probe = await ctx.taskModelRuntime.probe(route, signal);
				if (probe.supported === false) liveCheck = {
					id: "runtime.live-probe",
					status: "warn",
...
*/
const VOLCENGINE_ARK_DOCS = "https://docs.volcengine.com/api/doc/getDocDetail?LibraryID=82379&DocumentID=1330310&lang=zh";
const VOLCENGINE_ARK_PRICING_DOCS = "https://docs.volcengine.com/api/doc/getDocDetail?LibraryID=82379&DocumentID=1544106&lang=zh";
const DOUBAO_SPEECH_DOCS = "https://docs.volcengine.com/api/doc/getDocDetail?LibraryID=6561&DocumentID=2499930&lang=zh";
const DOUBAO_SPEECH_PRICING_DOCS = "https://docs.volcengine.com/api/doc/getDocDetail?LibraryID=6561&DocumentID=1359370&lang=zh";
const DOUBAO_REALTIME_DOCS = "https://docs.volcengine.com/api/doc/getDocDetail?LibraryID=6561&DocumentID=1594356&lang=zh";
const OPENAI_MODELS_DOCS = "https://developers.openai.com/api/docs/models/all";
const OPENAI_PRICING_DOCS = "https://developers.openai.com/api/docs/pricing";
const OPENAI_IMAGE_DOCS = "https://developers.openai.com/api/docs/guides/image-generation";
const OPENAI_VIDEO_DOCS = "https://developers.openai.com/api/docs/models/sora-2";
const ANTHROPIC_MODELS_DOCS = "https://platform.claude.com/docs/en/models/overview";
...
const GOOGLE_PRICING_DOCS = "https://ai.google.dev/gemini-api/docs/pricing?hl=en";
const GOOGLE_VIDEO_DOCS = "https://ai.google.dev/gemini-api/docs/video?hl=en";
...
*/
function officialResearchSources(provider, model, task) {
	const modelId = model?.trim();
	switch (provider) {
		case "volcengine": return [VOLCENGINE_ARK_DOCS, VOLCENGINE_ARK_PRICING_DOCS];
		case "doubao":
		case "doubao-speech": return [
			DOUBAO_SPEECH_DOCS,
			DOUBAO_SPEECH_PRICING_DOCS,
			DOUBAO_REALTIME_DOCS
		];
		case "openai":
		case "gpt-proxy": return uniqueSources([
			...modelId === void 0 ? [] : [`https://developers.openai.com/api/docs/models/${encodeURIComponent(modelId)}`],
			OPENAI_MODELS_DOCS,
			OPENAI_PRICING_DOCS,
		...
		case "google":
		case "google-vertex": return uniqueSources([
			...modelId === void 0 ? [] : [`https://ai.google.dev/gemini-api/docs/models/${encodeURIComponent(modelId)}?hl=en`],
			GOOGLE_MODELS_DOCS,
			GOOGLE_PRICING_DOCS,
			...task === "video-generation" ? [GOOGLE_VIDEO_DOCS] : []
		]);
		case "deepseek":
		case "deepseek-official": return [DEEPSEEK_MODELS_DOCS, DEEPSEEK_PRICING_DOCS];
		case "moonshotai":
		...
		case "minimax":
		case "minimax-cn": return uniqueSources([
			...
			...task === void 0 ? [MINIMAX_MODELS_DOCS, MINIMAX_MULTIMODAL_DOCS] : [],
...
function portraitGaps(portrait) {
	...
	if (portrait === void 0 || (portrait.pricing?.rates?.length ?? 0) === 0) gaps.push("pricing");
	if (portrait === void 0 || (portrait.specialties?.length ?? 0) === 0) gaps.push("specialties");
	if (portrait === void 0 || (portrait.limitations?.length ?? 0) === 0) gaps.push("limitations");
	if (portrait === void 0 || (portrait.evidence?.length ?? 0) === 0) gaps.push("evidence");
...
*/
function researchPlanFor(provider, gaps, context = {}) {
	...
	const suggestedSources = [.../* @__PURE__ */ new Set([...(context.evidenceSources ?? []).filter((source) => httpUrl(source) && officialSites.has(sourceSite(source))), ...officialSources])];
	const questions = RESEARCHABLE_GAPS.filter((field) => gaps.includes(field)).map((field) => ({
		field,
	...
	return {
		...
		questions,
		lastProbe: gaps.includes("lastProbe") ? "After explicit user approval, run validate_model_portrait with liveProbe=true. Do not copy documentation latency into lastProbe." : void 0
	};
}
/**
...
async function ingestPortraitResearch(ctx, input) {
	...
	if (findings.performance !== void 0 && Object.prototype.hasOwnProperty.call(findings.performance, "lastProbe")) throw new ModelManagerError("ingest_portrait_research cannot write lastProbe; only validate_model_portrait(liveProbe=true) may record an approved Agent live probe", "RESEARCH_CANNOT_WRITE_PROBE");
...
async function prepareModelPortraits(ctx, input, signal) {
	...
	for (const [id, registration] of Object.entries(config.models)) {
		...
		if (requested === void 0 && input.includeDisabled !== true) {
			if (!(await effectiveTaskModelAvailability(ctx, id)).enabled) continue;
		}
		const provider = config.connections[registration.connection]?.provider ?? registration.connection;
		const storedPortrait = registration.portrait === void 0 ? void 0 : normalizeStoredPortrait(registration.portrait);
		...
		candidates.push({
			...
			researchPlan: researchPlanFor(provider, gaps, {
				model: registration.model,
				task: registration.task,
				...portrait === void 0 ? {} : { evidenceSources: portrait.evidence.map((item) => item.source) }
			})
		});
	}
	const warnings = [];
	for (const provider of ctx.llm.listProviders()) try {
		for (const model of await ctx.llm.listModels(provider.id)) {
			...
			const storedPortrait = storedValue === void 0 ? void 0 : normalizeStoredPortrait(storedValue);
	...
	if (requested !== void 0) {
		...
		if (unknown.length > 0) throw new ModelManagerError(`portrait targets are unknown or not configured in this profile: ${unknown.join(", ")}`, "UNKNOWN_MODEL_PORTRAIT_TARGET");
	}
	return {
...
//#endregion
//#region lib/types/portraits/source-fetch.js
const MAX_SOURCE_BYTES = 2097152;
const MAX_SOURCE_CHARS = 8e4;
...
function portraitSourceText(body, contentType) {
	if (contentType.includes("json")) {
		const extracted = jsonDocumentText(body);
		if (extracted !== void 0) return extracted.slice(0, MAX_SOURCE_CHARS);
	}
	if (!contentType.includes("html")) return body.slice(0, MAX_SOURCE_CHARS);
	return decodeEntities(body.replace(/<(script|style|noscript|svg)\b[^>]*>[\s\S]*?<\/\1>/gi, " ").replace(/<!--([\s\S]*?)-->/g, " ").replace(/<br\s*\/?>/gi, "\n").replace(/<\/(?:p|div|li|h[1-6]|tr|section|article|main|nav)>/gi, "\n").replace(/<[^>]+>/g, " ")).replace(/[\t\f\v ]+/g, " ").replace(/ *\n */g, "\n").replace(/\n{3,}/g, "\n\n").trim().slice(0, MAX_SOURCE_CHARS);
}
function jsonDocumentText(body) {
	try {
		const value = JSON.parse(body);
		const record = value !== null && typeof value === "object" && !Array.isArray(value) ? value : void 0;
		const result = record?.Result;
		const resultRecord = result !== null && typeof result === "object" && !Array.isArray(result) ? result : void 0;
		const content = resultRecord?.Content ?? record?.content;
		const title = typeof resultRecord?.Title === "string" ? resultRecord.Title : void 0;
		if (typeof content === "string") return [title, nestedEditorText(content) ?? portraitSourceText(content, content.includes("<") ? "text/html" : "text/plain")].filter(Boolean).join("\n\n");
		return nestedEditorText(value);
	} catch {
		return;
	}
}
function nestedEditorText(value) {
	let parsed = value;
	...
	const visit = (current) => {
		...
		for (const [key, nested] of Object.entries(current)) if (key === "insert" && typeof nested === "string") inserts.push(nested);
		else visit(nested);
	};
	visit(parsed);
	if (inserts.length === 0) return void 0;
	return inserts.join("").replace(/\n{3,}/g, "\n\n").trim();
}
/** Collect the exact plugin-approved documentation entry points from a research manifest. */
function portraitResearchSources(value) {
	...
	const visit = (current) => {
		...
		for (const [key, nested] of Object.entries(current)) if (key === "suggestedSources" && Array.isArray(nested)) for (const source of nested) {
...
async function boundedSourceBytes(response) {
	const reader = response.body?.getReader();
	if (reader === void 0) return /* @__PURE__ */ new Uint8Array();
	const chunks = [];
	let size = 0;
	try {
		while (true) {
...
async function fetchPortraitSource(url, allowedSources, signal) {
	let normalized;
	try {
	...
	if (!allowedSources.has(normalized)) throw new Error("portrait source is not one of the plugin-approved documentation entry points");
	const timeout = AbortSignal.timeout(FETCH_TIMEOUT_MS);
	const combinedSignal = signal === void 0 ? timeout : AbortSignal.any([signal, timeout]);
	...
	if (!/(?:text\/|application\/(?:json|ld\+json|xhtml\+xml))/.test(contentType)) throw new Error(`portrait source returned unsupported content type: ${contentType}`);
	const bytes = await boundedSourceBytes(response);
	const content = portraitSourceText(new TextDecoder().decode(bytes), contentType);
	return {
...
async function fetchApprovedSource(approvedUrl, signal) {
	...
	for (let redirect = 0; redirect <= MAX_REDIRECTS; redirect += 1) {
		...
		if (next.protocol !== "https:" && next.protocol !== "http:" || siteBoundary(next.hostname) !== approvedSite) throw new Error("portrait source redirected outside its approved first-party site");
		current = next.href;
	}
	throw new Error("portrait source redirect resolution failed");
}
function siteBoundary(hostname) {
...
const discoveredModelProfileSchema = {
	...
	properties: {
		...
		maxTokens: {
			type: "integer",
			description: "Discovered maximum output capacity; it is not a per-request output limit."
		},
		input: {
			type: "array",
			items: {
...
const portraitSchema = {
	...
	properties: {
		description: {
			type: "string",
			description: "Sectioned Markdown description for qualitative routing knowledge."
		},
		summary: {
			type: "string",
		...
		defineTool({
			...
			parameters: {
				id: {
					type: "string",
					required: true,
					description: FETCH_PORTRAIT_SOURCE_SURFACE.parameters.id
				},
				url: {
					type: "string",
					required: true,
					description: FETCH_PORTRAIT_SOURCE_SURFACE.parameters.url
				}
			},
			output: jsonOutput,
...
//#endregion
//#region lib/types/model/portrait-skill.js
const MODEL_PORTRAIT_SKILL_NAME = "collect-model-portraits";
const MODEL_PORTRAIT_SKILL_DESCRIPTION = "Research and update evidence-backed portraits for models already configured in the current DSH profile. Use when the user asks to collect, organize, create, refresh, or improve model portraits; do not use for unconfigured catalog entries.";
function skillDocumentUrl() {
...
function modelPortraitSkillContent(url = skillDocumentUrl()) {
...
function registerModelPortraitSkill(ctx) {
...
function apply(ctx) {
	ctx.effect(async () => {
		try {
			if (await migrateLegacyVolcengineCredential(ctx)) ctx.logger.info("migrated legacy VOLCENGINE_API_KEY credential reference to ARK_API_KEY");
		} catch (error) {
	...
	ctx.llm.registerModelDiscovery("multi-model-provider", async (request) => {
		if (request.provider === "volcengine") return discoverVolcengineModels(ctx, request);
		if (request.provider !== "doubao-speech") return [];
		const stored = request.apiKey === void 0 ? await ctx.credentials.resolve(credentialRef("DOUBAO_API_KEY")) : void 0;
		return discoverDoubaoRealtimeVoices({
			...request,
			...stored === void 0 ? {} : { apiKey: stored.value }
		});
	});
...
//#endregion
export { ANTHROPIC_MODELS_DOCS, ANTHROPIC_PRICING_DOCS, BUILTIN_TASK_MODEL_REGISTRY, CONFIGURE_MODEL_ROUTE_SURFACE, CURATED_LLM_PORTRAIT_IDS, CURATED_LLM_PORTRAIT_SELECTION, CURATED_PORTABLE_LLM_MODEL_IDS, CURATED_TASK_MODEL_PORTRAIT_IDS, CURATED_TASK_MODEL_PORTRAIT_SELECTION, DEEPSEEK_MODELS_DOCS, DEEPSEEK_PRICING_DOCS, DISCOVER_TASK_MODELS_SURFACE, DOUBAO_REALTIME_DOCS, DOUBAO_SPEECH_DOCS, DOUBAO_SPEECH_PRICING_DOCS, FETCH_PORTRAIT_SOURCE_SURFACE, GET_MODEL_PORTRAIT_SURFACE, GOOGLE_MODELS_DOCS, GOOGLE_PRICING_DOCS, GOOGLE_VIDEO_DOCS, HELP, INGEST_PORTRAIT_RESEARCH_SURFACE, INSPECT_VOLCENGINE_PROVIDER_SURFACE, INVOKE_TASK_MODEL_SURFACE, KIMI_MODELS_DOCS, KIMI_PRICING_DOCS, LEGACY_VOLCENGINE_ARK_API_KEY, LIST_MODEL_ROUTES_SURFACE, LIST_TASK_MODELS_SURFACE, MINIMAX_H3_DOCS, MINIMAX_H3_OPEN_SOURCE_DOCS, MINIMAX_IMAGE_DOCS, MINIMAX_LOCAL_DEPLOY_DOCS, MINIMAX_MODELS_DOCS, MINIMAX_MULTIMODAL_DOCS, MINIMAX_PRICING_DOCS, MISTRAL_MODELS_DOCS, MISTRAL_PRICING_DOCS, MODEL_CATALOG_PATH, MODEL_EXECUTION_MODES, MODEL_MANAGER_GUIDANCE, MODEL_MANAGER_TOOL_SURFACES, MODEL_MODALITIES, MODEL_PORTRAIT_SKILL_DESCRIPTION, MODEL_PORTRAIT_SKILL_NAME, ModelCatalog, ModelManagerError, OPENAI_IMAGE_DOCS, OPENAI_MODELS_DOCS, OPENAI_PRICING_DOCS, OPENAI_VIDEO_DOCS, PI_AI_SETTINGS_NAMESPACE, PREPARE_MODEL_PORTRAITS_SURFACE, QWEN_MODELS_DOCS, QWEN_OPEN_WEIGHTS_DOCS, QWEN_PRICING_DOCS, REGISTER_TASK_MODEL_SURFACE, RealtimeModelRuntime, SELECT_DEFAULT_MODEL_SURFACE, SELECT_TASK_MODELS_SURFACE, SELECT_VOLCENGINE_LANGUAGE_MODELS_SURFACE, SUMMARIZE_MODEL_USAGE_SURFACE, TASK_MODEL_CAPABILITIES, TASK_MODEL_REGISTRY_SCHEMA, TASK_MODEL_SETTINGS_NAMESPACE, TASK_MODEL_TASKS, TOOL_NAMES, TaskModelRuntime, TaskPipelineRuntime, UPSERT_MODEL_PORTRAIT_SURFACE, VALIDATE_MODEL_PORTRAIT_SURFACE, VERSION, VOLCENGINE_AGENT_PLAN_PROVIDER, VOLCENGINE_ARK_API, VOLCENGINE_ARK_API_KEY, VOLCENGINE_ARK_BASE_URL, VOLCENGINE_ARK_DOCS, VOLCENGINE_ARK_PAYG_BASE_URL, VOLCENGINE_ARK_PLAN_BASE_URL, VOLCENGINE_ARK_PRICING_DOCS, VOLCENGINE_PROVIDER, XAI_MODELS_DOCS, XAI_PRICING_DOCS, ZAI_MODELS_DOCS, ZAI_PRICING_DOCS, apply, assertSafeCatalogEndpoint, builtinLlmPortrait, builtinTaskPortrait, configureModelRoute, configuredTaskModelIds, discoverTaskModels, discoverVolcengineModels, effectiveTaskModelAvailability, getModelPortrait, ingestPortraitResearch, initialPortrait, inject, inspectVolcengineProvider, invokeTaskModel, listModelRoutes, listTaskModels, llmObservations, migrateLegacyVolcengineCredential, modelManagerTools, modelPortraitSkillContent, mutatePortraitSettings, name, normalizePortrait, normalizeStoredPortrait, officialResearchSources, portraitChecks, portraitGaps, portraitRegistry, portraitSettings, prepareModelPortraits, recordTaskModelObservation, registerModelCatalogRoute, registerModelPortraitSkill, registerTaskModel, registerTaskModelSettings, researchPlanFor, resolvePortraitTarget, resolveTaskModelRoute, selectAgentModel, selectDefaultModel, selectTaskModels, selectVolcengineLanguageModels, snapshotModelCatalog, summarizeModelUsage, taskModelObservations, upsertModelPortrait, validateModelPortrait, validateTaskModelRegistry };
