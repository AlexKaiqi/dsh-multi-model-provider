...
async function inspectVolcengineProvider(ctx, signal) {
	...
	const taskRoutes = (Array.isArray(listedTasks.models) ? listedTasks.models : []).map((value) => {
		...
		const availability = object$1(route.availability) ?? {};
		return {
			id: route.id,
			model: route.model,
			task: route.task,
...
async function validateModelPortrait(ctx, input, signal) {
	...
	if (input.liveProbe === true) {
		if (route.registration.execution === "realtime") checks.push({
			id: "runtime.live-probe",
			status: "warn",
