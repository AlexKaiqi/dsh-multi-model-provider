...
var PortraitJobCoordinator = class {
	...
	async start(action, ids, approved) {
		...
		if (selectedIds.length > 0) this.execute(prompt, action, researchSources);
		return publicJob(this.latest);
	}
	async dispose() {
		this.disposed = true;
	...
	async execute(prompt, action, researchSources) {
		...
		try {
			...
			if (workspace !== void 0) await rm(workspace, {
				recursive: true,
				force: true
			}).catch(() => void 0);
			if (failure === void 0) {
			...
			const unregister = jobScope.webServer.register({
				...
				handler: async (req, res) => {
					...
					try {
						...
						sendJson(res, 202, {
							ok: true,
							job: await coordinator.start(body.action, targetIds(body.ids), body.approved === true)
						});
					} catch (error) {
