...
function providerModelSelection(descriptor, connectionId) {
	...
	const connections = descriptor.user.connections;
...
var PortraitJobCoordinator = class {
	...
	async start(action, ids, approved) {
		...
		this.latest = {
			...
			...selectedIds.length === 0 ? {
				status: "completed",
				finishedAt: now,
				summary: "No enabled portraits currently need research."
			} : {}
		};
	...
	async execute(prompt, action, researchSources, signal) {
		...
		try {
			...
			if (signal.aborted) throw new Error("portrait background job was cancelled");
			if (!(await this.ctx.temporarySessions.keep({ reservationId: reservation.reservationId })).found) throw new Error("temporary Session reservation expired before adoption");
			...
			if (reservation !== void 0 && !reservationAdopted) await this.ctx.temporarySessions.discard({ reservationId: reservation.reservationId }).catch(() => void 0);
			if (failure === void 0) {
...
function decorateConfigurableProviderView(view) {
	if (view.provider !== "volcengine") return void 0;
	return {
		...view,
		displayName: "火山方舟",
		declared: false,
...
function apply(ctx) {
	...
	if (!existingProviders.has("volcengine")) arkDirectory = ctx.llm.registerConfigurableProviders([{
		provider: "volcengine",
		displayName: "火山方舟",
		settingsNs: "llm-pi-ai",
		settingsPath: ["providers", "volcengine"],
