...
//#endregion
//#region lib/types/provider-directory.js
...
function doubaoProviderDirectoryEntry() {
	return {
		provider: DOUBAO_SPEECH_PROVIDER,
		displayName: "豆包语音",
...
function volcengineProviderDirectoryEntry() {
	return {
		provider: "volcengine",
		displayName: "火山方舟",
...
function apply(ctx) {
	...
	if (!existingProviders.has("volcengine")) arkDirectory = ctx.llm.registerConfigurableProviders([volcengineProviderDirectoryEntry()]);
	ctx.on("llm/adapters-updated", ensureArkDirectory);
	ensureArkDirectory();
	if (!existingProviders.has("doubao-speech")) ctx.llm.registerConfigurableProviders([doubaoProviderDirectoryEntry()]);
