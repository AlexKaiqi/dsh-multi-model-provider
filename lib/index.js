...
async function listTaskModels(ctx, input = {}) {
	...
	const providerSelections = /* @__PURE__ */ new Map();
	const models = [];
	for (const [id, model] of Object.entries(config.models)) {
		...
		if (!providerSelections.has(model.connection)) providerSelections.set(model.connection, providerModelSelection(descriptor, model.connection));
		const providerSelection = providerSelections.get(model.connection);
		const enabled = providerSelection === void 0 ? model.enabled !== false : selectedByProviderEditor(providerSelection, id, model);
