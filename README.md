...
On DSH `0.1.2-rc.1` and later, open **Settings → Models → Configure Volcengine Ark**. The plugin uses the native `settings.models.footer` extension slot. The form fills in the official pay-as-you-go Base URL `https://ark.cn-beijing.volces.com/api/v3`; enter an API key, fetch the account catalog, select model IDs, then save. Listing errors remain visible, and exact model IDs or `ep-*` endpoint IDs can also be entered manually. Saving requires at least one model because an unknown pi-ai provider has no built-in model catalog. Discovered output capacity is not saved as a request token limit. Saved routes appear in the ordinary provider rows and session model selector.

The form reads and writes through DSH's settings, credentials, and LLM Remotes. Settings writes carry the observed revision and update only the fields the form owns. Keys stay in the credential store. A saved key is not forwarded to a changed endpoint during discovery; enter a key explicitly for that destination. On older hosts without the native slot, the Agent tools remain available.

The separate read-only **Model portraits** page has collect and view tabs. It lists configured models and runs the bundled `collect-model-portraits` skill in the current Session.

The plugin exposes two coexisting Ark providers: `volcengine` uses the pay-as-you-go `https://ark.cn-beijing.volces.com/api/v3` base URL, while `volcengine-agent-plan` uses `https://ark.cn-beijing.volces.com/api/plan/v3`. Both reference `ARK_API_KEY`, but their model selections and Session routes remain independent and neither silently falls back to the other. On upgrade, a legacy `VOLCENGINE_API_KEY` value is safely copied to the standard `ARK_API_KEY` reference when the latter is missing; the legacy reference is retained and the secret is never logged or written to ordinary settings. Doubao keeps the fixed Realtime protocol model `1.2.6.1` and voice as separate task-profile fields. The Ark setup form configures the pay-as-you-go route; Agent Plan remains independently configurable through the Agent tools. Doubao registration and selection remain in the task catalog and Agent tools.
